...
import { useGetBillsPaymentMethodsQuery } from "@/redux/reducers/Common/Home/BillsAnalyticsApi";
import { BillsPaymentMethodsPoint } from "@/types/Common/Home/BillsAnalyticsTypes";
...
const BillsPaymentMethods: React.FC = () => {
  const { data, isLoading, isError } =
    useGetBillsPaymentMethodsQuery(undefined);
  const chartData: BillsPaymentMethodsPoint[] = data ?? [];
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-3 max-w-2xl text-sm leading-6">
            Analyze payment method totals and volume across all bills.
          </p>
        </div>
      </div>
        ...
        <CardContent className="px-4 pt-4 pb-6 sm:px-6">
          {isLoading ? (
            ...
            <div className="border-destructive/10 bg-destructive/5 text-destructive rounded-[1.75rem] border p-6 text-sm">
              Unable to load billing payment method data.
            </div>
          ) : chartData.length === 0 ? (
...
export default BillsPaymentMethods;
